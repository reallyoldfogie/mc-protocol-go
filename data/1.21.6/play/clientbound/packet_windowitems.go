// Generated by gen_protocol. DO NOT EDIT
package clientbound

import (
	"fmt"
	pk "github.com/Tnze/go-mc/net/packet"
	"github.com/pkg/errors"
	"github.com/reallyoldfogie/mc-protocol-go/data/1.21.6/basetypes"
	"github.com/reallyoldfogie/mc-protocol-go/models"
	"io"
	"log"
)

// Protodef: [
//
//	  "container",
//	  [
//	    {
//	      "name": "windowId",
//	      "type": "ContainerID"
//	    },
//	    {
//	      "name": "stateId",
//	      "type": "varint"
//	    },
//	    {
//	      "name": "items",
//	      "type": [
//	        "array",
//	        {
//	          "countType": "varint",
//	          "type": "Slot"
//	        }
//	      ]
//	    },
//	    {
//	      "name": "carriedItem",
//	      "type": "Slot"
//	    }
//	  ]
//	]
type WindowItems struct {
	packetID int32
	// "ContainerID"
	WindowId basetypes.ContainerID
	// "varint"
	StateId pk.VarInt
	// [
	//                 "array",
	//                 {
	//                   "countType": "varint",
	//                   "type": "Slot"
	//                 }
	//               ]
	Items models.Array[pk.VarInt, basetypes.Slot]
	// "Slot"
	CarriedItem basetypes.Slot
}

// NewWindowItems creates a new WindowItems packet with the correct packet ID.
func NewWindowItems() *WindowItems {
	return &WindowItems{packetID: 18}
}

// PacketID returns the protocol ID for this packet type.
func (p *WindowItems) PacketID() int32 {
	return p.packetID
}

// SetPacketID sets the protocol ID for this packet type.
// This is used when the same packet structure is reused across multiple stages with different IDs.
func (p *WindowItems) SetPacketID(id int32) {
	p.packetID = id
}

// Marshal serializes the packet into wire format.
func (p *WindowItems) Marshal() pk.Packet {
	return pk.Marshal(
		p.packetID,
		&p.WindowId,
		&p.StateId,
		&p.Items,
		&p.CarriedItem)
}

// Scan deserializes a wire-format packet into this struct.
func (p *WindowItems) Scan(packet pk.Packet) error {
	if packet.ID != p.packetID {
		return fmt.Errorf("packet ID mismatch: expected %d, got %d", p.packetID, packet.ID)
	}
	return packet.Scan(
		&p.WindowId,
		&p.StateId,
		&p.Items,
		&p.CarriedItem)
}

// GetFields returns a map of all packet fields for version-agnostic access.
// Use this when you need to access fields dynamically or when working with version-specific types
// that don't have stable cross-version interfaces.
//
// For version-specific code with type safety, use the typed getter methods (e.g., GetCount()).
// For semi-agnostic code with fields that have stable types, use the typed interfaces (e.g., CountGetter).
func (p *WindowItems) GetFields() map[string]pk.FieldEncoder {
	fields := map[string]pk.FieldEncoder{}
	fields["WindowId"] = &p.WindowId
	fields["StateId"] = &p.StateId
	fields["Items"] = &p.Items
	fields["CarriedItem"] = &p.CarriedItem
	return fields
}

// SetFields updates packet fields from a map for version-agnostic access.
// Use this when you need to set fields dynamically or when working with version-specific types
// that don't have stable cross-version interfaces.
//
// For version-specific code with type safety, use the typed setter methods (e.g., SetCount()).
// For semi-agnostic code with fields that have stable types, use the typed interfaces (e.g., CountSetter).
func (p *WindowItems) SetFields(fields map[string]pk.FieldEncoder) {
	if val, ok := fields["WindowId"]; ok {
		p.WindowId = *val.(*basetypes.ContainerID)
	}
	if val, ok := fields["StateId"]; ok {
		p.StateId = *val.(*pk.VarInt)
	}
	if val, ok := fields["Items"]; ok {
		p.Items = *val.(*models.Array[pk.VarInt, basetypes.Slot])
	}
	if val, ok := fields["CarriedItem"]; ok {
		p.CarriedItem = *val.(*basetypes.Slot)
	}
}

// Typed field accessor methods for version-specific type-safe access
// GetWindowId returns the WindowId field value.
// Note: This method returns the actual field type, which may be version-specific.
// For version-agnostic access, use GetFields() or check for typed interfaces.
func (p *WindowItems) GetWindowId() basetypes.ContainerID {
	return p.WindowId
}

// SetWindowId sets the WindowId field value.
// Note: This method accepts the actual field type, which may be version-specific.
// For version-agnostic access, use SetFields() or check for typed interfaces.
func (p *WindowItems) SetWindowId(val basetypes.ContainerID) {
	p.WindowId = val
}

// GetStateId returns the StateId field value.
// Note: This method returns the actual field type, which may be version-specific.
// For version-agnostic access, use GetFields() or check for typed interfaces.
func (p *WindowItems) GetStateId() pk.VarInt {
	return p.StateId
}

// SetStateId sets the StateId field value.
// Note: This method accepts the actual field type, which may be version-specific.
// For version-agnostic access, use SetFields() or check for typed interfaces.
func (p *WindowItems) SetStateId(val pk.VarInt) {
	p.StateId = val
}

// GetItems returns the Items field value.
// Note: This method returns the actual field type, which may be version-specific.
// For version-agnostic access, use GetFields() or check for typed interfaces.
func (p *WindowItems) GetItems() models.Array[pk.VarInt, basetypes.Slot] {
	return p.Items
}

// SetItems sets the Items field value.
// Note: This method accepts the actual field type, which may be version-specific.
// For version-agnostic access, use SetFields() or check for typed interfaces.
func (p *WindowItems) SetItems(val models.Array[pk.VarInt, basetypes.Slot]) {
	p.Items = val
}

// GetCarriedItem returns the CarriedItem field value.
// Note: This method returns the actual field type, which may be version-specific.
// For version-agnostic access, use GetFields() or check for typed interfaces.
func (p *WindowItems) GetCarriedItem() basetypes.Slot {
	return p.CarriedItem
}

// SetCarriedItem sets the CarriedItem field value.
// Note: This method accepts the actual field type, which may be version-specific.
// For version-agnostic access, use SetFields() or check for typed interfaces.
func (p *WindowItems) SetCarriedItem(val basetypes.Slot) {
	p.CarriedItem = val
}

func (t *WindowItems) ReadFrom(r io.Reader) (totalBytes int64, err error) {
	var bytesRead int64
	bytesRead, err = t.WindowId.ReadFrom(r)

	totalBytes += bytesRead
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field WindowId")
	}
	bytesRead, err = t.StateId.ReadFrom(r)

	totalBytes += bytesRead
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field StateId")
	}
	bytesRead, err = t.Items.ReadFrom(r)

	totalBytes += bytesRead
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field Items")
	}
	bytesRead, err = t.CarriedItem.ReadFrom(r)

	totalBytes += bytesRead
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field CarriedItem")
	}

	return totalBytes, nil
}

func (t WindowItems) WriteTo(w io.Writer) (totalBytes int64, err error) {
	var bytesWritten int64

	defer func() {
		log.Printf("[WindowItems.WriteTo] totalBytes: %d err: %#v", totalBytes, err)
	}()
	bytesWritten, err = t.WindowId.WriteTo(w)
	totalBytes += bytesWritten
	if err != nil {
		return totalBytes, err
	}
	bytesWritten, err = t.StateId.WriteTo(w)
	totalBytes += bytesWritten
	if err != nil {
		return totalBytes, err
	}
	bytesWritten, err = t.Items.WriteTo(w)
	totalBytes += bytesWritten
	if err != nil {
		return totalBytes, err
	}
	bytesWritten, err = t.CarriedItem.WriteTo(w)
	totalBytes += bytesWritten
	if err != nil {
		return totalBytes, err
	}
	return totalBytes, nil
}
