// Generated by gen_protocol. DO NOT EDIT
package serverbound

import (
	"fmt"
	pk "github.com/Tnze/go-mc/net/packet"
	"github.com/pkg/errors"
	"io"
	"log"
)

// Protodef: [
//
//	  "container",
//	  [
//	    {
//	      "name": "entityId",
//	      "type": "varint"
//	    },
//	    {
//	      "name": "actionId",
//	      "type": [
//	        "mapper",
//	        {
//	          "type": "varint",
//	          "mappings": {
//	            "0": "leave_bed",
//	            "1": "start_sprinting",
//	            "2": "stop_sprinting",
//	            "3": "start_horse_jump",
//	            "4": "stop_horse_jump",
//	            "5": "open_vehicle_inventory",
//	            "6": "start_elytra_flying"
//	          }
//	        }
//	      ]
//	    },
//	    {
//	      "name": "jumpBoost",
//	      "type": "varint"
//	    }
//	  ]
//	]
type EntityAction struct {
	packetID int32
	// "varint"
	EntityId pk.VarInt
	// [
	//                 "mapper",
	//                 {
	//                   "type": "varint",
	//                   "mappings": {
	//                     "0": "leave_bed",
	//                     "1": "start_sprinting",
	//                     "2": "stop_sprinting",
	//                     "3": "start_horse_jump",
	//                     "4": "stop_horse_jump",
	//                     "5": "open_vehicle_inventory",
	//                     "6": "start_elytra_flying"
	//                   }
	//                 }
	//               ]
	ActionId EntityActionActionId
	// "varint"
	JumpBoost pk.VarInt
}

// NewEntityAction creates a new EntityAction packet with the correct packet ID.
func NewEntityAction() *EntityAction {
	return &EntityAction{packetID: 41}
}

// PacketID returns the protocol ID for this packet type.
func (p *EntityAction) PacketID() int32 {
	return p.packetID
}

// SetPacketID sets the protocol ID for this packet type.
// This is used when the same packet structure is reused across multiple stages with different IDs.
func (p *EntityAction) SetPacketID(id int32) {
	p.packetID = id
}

// Marshal serializes the packet into wire format.
func (p *EntityAction) Marshal() pk.Packet {
	return pk.Marshal(
		p.packetID,
		&p.EntityId,
		&p.ActionId,
		&p.JumpBoost)
}

// Scan deserializes a wire-format packet into this struct.
func (p *EntityAction) Scan(packet pk.Packet) error {
	if packet.ID != p.packetID {
		return fmt.Errorf("packet ID mismatch: expected %d, got %d", p.packetID, packet.ID)
	}
	return packet.Scan(
		&p.EntityId,
		&p.ActionId,
		&p.JumpBoost)
}

// GetFields returns a map of all packet fields for version-agnostic access.
// Use this when you need to access fields dynamically or when working with version-specific types
// that don't have stable cross-version interfaces.
//
// For version-specific code with type safety, use the typed getter methods (e.g., GetCount()).
// For semi-agnostic code with fields that have stable types, use the typed interfaces (e.g., CountGetter).
func (p *EntityAction) GetFields() map[string]pk.FieldEncoder {
	fields := map[string]pk.FieldEncoder{}
	fields["EntityId"] = &p.EntityId
	fields["ActionId"] = &p.ActionId
	fields["JumpBoost"] = &p.JumpBoost
	return fields
}

// SetFields updates packet fields from a map for version-agnostic access.
// Use this when you need to set fields dynamically or when working with version-specific types
// that don't have stable cross-version interfaces.
//
// For version-specific code with type safety, use the typed setter methods (e.g., SetCount()).
// For semi-agnostic code with fields that have stable types, use the typed interfaces (e.g., CountSetter).
func (p *EntityAction) SetFields(fields map[string]pk.FieldEncoder) {
	if val, ok := fields["EntityId"]; ok {
		p.EntityId = *val.(*pk.VarInt)
	}
	if val, ok := fields["ActionId"]; ok {
		p.ActionId = *val.(*EntityActionActionId)
	}
	if val, ok := fields["JumpBoost"]; ok {
		p.JumpBoost = *val.(*pk.VarInt)
	}
}

// Typed field accessor methods for version-specific type-safe access
// GetEntityId returns the EntityId field value.
// Note: This method returns the actual field type, which may be version-specific.
// For version-agnostic access, use GetFields() or check for typed interfaces.
func (p *EntityAction) GetEntityId() pk.VarInt {
	return p.EntityId
}

// SetEntityId sets the EntityId field value.
// Note: This method accepts the actual field type, which may be version-specific.
// For version-agnostic access, use SetFields() or check for typed interfaces.
func (p *EntityAction) SetEntityId(val pk.VarInt) {
	p.EntityId = val
}

// GetActionId returns the ActionId field value.
// Note: This method returns the actual field type, which may be version-specific.
// For version-agnostic access, use GetFields() or check for typed interfaces.
func (p *EntityAction) GetActionId() EntityActionActionId {
	return p.ActionId
}

// SetActionId sets the ActionId field value.
// Note: This method accepts the actual field type, which may be version-specific.
// For version-agnostic access, use SetFields() or check for typed interfaces.
func (p *EntityAction) SetActionId(val EntityActionActionId) {
	p.ActionId = val
}

// GetJumpBoost returns the JumpBoost field value.
// Note: This method returns the actual field type, which may be version-specific.
// For version-agnostic access, use GetFields() or check for typed interfaces.
func (p *EntityAction) GetJumpBoost() pk.VarInt {
	return p.JumpBoost
}

// SetJumpBoost sets the JumpBoost field value.
// Note: This method accepts the actual field type, which may be version-specific.
// For version-agnostic access, use SetFields() or check for typed interfaces.
func (p *EntityAction) SetJumpBoost(val pk.VarInt) {
	p.JumpBoost = val
}

func (t *EntityAction) ReadFrom(r io.Reader) (totalBytes int64, err error) {
	var bytesRead int64
	bytesRead, err = t.EntityId.ReadFrom(r)

	totalBytes += bytesRead
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field EntityId")
	}
	bytesRead, err = t.ActionId.ReadFrom(r)

	totalBytes += bytesRead
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field ActionId")
	}
	bytesRead, err = t.JumpBoost.ReadFrom(r)

	totalBytes += bytesRead
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field JumpBoost")
	}

	return totalBytes, nil
}

func (t EntityAction) WriteTo(w io.Writer) (totalBytes int64, err error) {
	var bytesWritten int64

	defer func() {
		log.Printf("[EntityAction.WriteTo] totalBytes: %d err: %#v", totalBytes, err)
	}()
	bytesWritten, err = t.EntityId.WriteTo(w)
	totalBytes += bytesWritten
	if err != nil {
		return totalBytes, err
	}
	bytesWritten, err = t.ActionId.WriteTo(w)
	totalBytes += bytesWritten
	if err != nil {
		return totalBytes, err
	}
	bytesWritten, err = t.JumpBoost.WriteTo(w)
	totalBytes += bytesWritten
	if err != nil {
		return totalBytes, err
	}
	return totalBytes, nil
}
