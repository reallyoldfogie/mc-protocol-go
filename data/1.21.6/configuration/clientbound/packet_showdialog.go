...
func (p *ShowDialog) SetFields(fields map[string]pk.FieldEncoder) {
	if val, ok := fields["Dialog"]; ok {
		p.Dialog = *val.(*models.AnonymousNBT)
	}
}
...
func (p *ShowDialog) GetDialog() models.AnonymousNBT {
	return p.Dialog
}
...
func (p *ShowDialog) SetDialog(val models.AnonymousNBT) {
	p.Dialog = val
}
