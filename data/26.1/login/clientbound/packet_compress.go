...
func (p *Compress) SetPacketID(id int32) {
	p.packetID = id
}
...
func (p *Compress) GetThreshold() pk.VarInt {
	return p.Threshold
}

// SetThreshold sets the Threshold field value.
...
func (p *Compress) SetThreshold(val pk.VarInt) {
	p.Threshold = val
}
...
func (t Compress) WriteTo(w io.Writer) (totalBytes int64, err error) {
	...
	defer func() {
		log.Printf("[Compress.WriteTo] totalBytes: %d err: %#v", totalBytes, err)
	}()
