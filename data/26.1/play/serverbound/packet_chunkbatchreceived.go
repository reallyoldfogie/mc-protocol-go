...
func NewChunkBatchReceived() *ChunkBatchReceived {
	return &ChunkBatchReceived{packetID: 11}
}
...
func (p *ChunkBatchReceived) SetPacketID(id int32) {
	p.packetID = id
}
...
// GetChunksPerTick returns the ChunksPerTick field value.
...
func (p *ChunkBatchReceived) GetChunksPerTick() pk.Float {
	return p.ChunksPerTick
}

// SetChunksPerTick sets the ChunksPerTick field value.
...
func (p *ChunkBatchReceived) SetChunksPerTick(val pk.Float) {
	p.ChunksPerTick = val
}
...
func (t *ChunkBatchReceived) ReadFrom(r io.Reader) (totalBytes int64, err error) {
	...
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field ChunksPerTick")
	}
...
func (t ChunkBatchReceived) WriteTo(w io.Writer) (totalBytes int64, err error) {
	...
	defer func() {
		log.Printf("[ChunkBatchReceived.WriteTo] totalBytes: %d err: %#v", totalBytes, err)
	}()
