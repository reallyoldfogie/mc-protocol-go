...
// NewCodeOfConduct creates a new CodeOfConduct packet with the correct packet ID.
func NewCodeOfConduct() *CodeOfConduct {
	return &CodeOfConduct{packetID: 19}
}

// PacketID returns the protocol ID for this packet type.
func (p *CodeOfConduct) PacketID() int32 {
	return p.packetID
}
...
func (p *CodeOfConduct) SetPacketID(id int32) {
	p.packetID = id
}

// Marshal serializes the packet into wire format.
func (p *CodeOfConduct) Marshal() pk.Packet {
	return pk.Marshal(
		p.packetID,
		&p.Contents)
}
...
func (p *CodeOfConduct) Scan(packet pk.Packet) error {
...
func (p *CodeOfConduct) GetFields() map[string]pk.FieldEncoder {
...
func (p *CodeOfConduct) SetFields(fields map[string]pk.FieldEncoder) {
	fmt.Printf("<no value>\n")
	if val, ok := fields["Contents"]; ok {
		p.Contents = val.(pk.String)
	}
}
...
func (p *CodeOfConduct) GetContents() pk.String {
	return p.Contents
}
...
func (p *CodeOfConduct) SetContents(val pk.String) {
	p.Contents = val
}
...
func (t CodeOfConduct) WriteTo(w io.Writer) (totalBytes int64, err error) {
	var bytesWritten int64

	defer func() {
		log.Printf("[CodeOfConduct.WriteTo] totalBytes: %d err: %#v", totalBytes, err)
	}()
