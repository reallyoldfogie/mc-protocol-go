...
func (p *SimulationDistance) SetPacketID(id int32) {
	p.packetID = id
}
...
func (p *SimulationDistance) GetDistance() pk.VarInt {
	return p.Distance
}

// SetDistance sets the Distance field value.
...
func (p *SimulationDistance) SetDistance(val pk.VarInt) {
	p.Distance = val
}
...
func (t *SimulationDistance) ReadFrom(r io.Reader) (totalBytes int64, err error) {
	...
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field Distance")
	}
...
func (t SimulationDistance) WriteTo(w io.Writer) (totalBytes int64, err error) {
	...
	defer func() {
		log.Printf("[SimulationDistance.WriteTo] totalBytes: %d err: %#v", totalBytes, err)
	}()
