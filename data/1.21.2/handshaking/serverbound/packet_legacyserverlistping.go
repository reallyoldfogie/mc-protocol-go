...
func (p *LegacyServerListPing) SetPacketID(id int32) {
	p.packetID = id
}
...
func (p *LegacyServerListPing) GetPayload() pk.UnsignedByte {
	return p.Payload
}
...
func (p *LegacyServerListPing) SetPayload(val pk.UnsignedByte) {
	p.Payload = val
}
...
func (t *LegacyServerListPing) ReadFrom(r io.Reader) (totalBytes int64, err error) {
	...
	if err != nil {
		return totalBytes, errors.Wrap(err, "failed to read field Payload")
	}
...
func (t LegacyServerListPing) WriteTo(w io.Writer) (totalBytes int64, err error) {
	...
	defer func() {
		log.Printf("[LegacyServerListPing.WriteTo] totalBytes: %d err: %#v", totalBytes, err)
	}()
